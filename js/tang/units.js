...
// --- 1 metre = 1/40,000,000 of the terrestrial equator: the metre is itself an
//     Earth unit, so converting our angle/distance ratios through it returns
//     the accepted dimensions of Earth, self-consistently --------------------
// NOTHING in the sky physics uses Earth's radius. The ephemerides run entirely
// on du angles and dimensionless ratios — parallax is R/d as a RATIO, distances
// come out as multiples of R — so R is never needed to compute a position. R is
// needed only to express those ratios in a familiar unit AFTER the fact.
//
// That familiar unit is itself defined from Earth: the metre was fixed as
// 1/40,000,000 of the terrestrial equator (great-circle circumference). So the
// li↔km bridge is just two Earth-circumference units compared, and pushing our
// du/li ratios through it reproduces the SAME internal consistency that is today
// reported as the accepted dimensions of Earth — a metre defined from Earth,
// measuring Earth, returns Earth. We anchor on the accepted mean radius
// (6,371 km), take the great-circle circumference from it, and let the
// km-per-li factor and the radius itself FALL OUT of the Chinese li circle:
// EARTH_RADIUS_KM is DERIVED (= R_LI × KM_PER_LI) and equals 6,371 by
// construction, not a hand-entered number. Change this one anchor and every
// km/mile readout rescales; no angle moves.
const EARTH_MEAN_RADIUS_KM     = 6371;                         // anchor: accepted mean radius
export const KM_PER_LI         = (2 * Math.PI * EARTH_MEAN_RADIUS_KM) / TANG_CIRCUMFERENCE_LI;
export const EARTH_RADIUS_KM   = R_LI * KM_PER_LI;             // DERIVED → 6371.0 km
export const EARTH_RADIUS_MI   = EARTH_RADIUS_KM / 1.609344;   // Imperial readout
export const MOON_MEAN_DIST_KM = 385000.56;          // Meeus lunar mean (Σr normaliser, intrinsic)
export const MOON_MEAN_DIST_M  = MOON_MEAN_DIST_KM * 1000;

// Speed of light as a li-native speed: c_SI ÷ (metres per li). This is the
// units_of_measurement li light value. It is used only for light-time, where
// the quantity that matters is AU/c — a TIME, the scale-invariant the brief
// identifies — so no length unit survives into the result.
export const C_LI_PER_SEC = 299792458 / (KM_PER_LI * 1000);   // ≈ 9.6×10⁵ li/s
